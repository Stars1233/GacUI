...
- Although most types have a `Gui` prefix, all TUI related types should have a `Tui` prefix.
  ...
  - Controller source code will be in `Source/PlatformProviders/Windows/TUI`.
...
- Renderer:
  - `ImageFrame` completedly not implemented, by not setting a renderer factory, using `<ImageFrame/>` leading to crash is expected, due to not offering the renderer.
    - Also no `Polygon`.
  - `SolidLabel` and `IGuiGraphicsParagraph` should be implemented but surely to ignore `fontFamily` and `size` in `FontProperties`.
...
- TUI on GacUI
  ...
  - Maximize/Minimize/Restore/Close buttons on the main window will always exists, this can be controlled by the window template offered to the main window, therefore setting them to false won't be effective.
    ...
    - Resizing, maximizing, minimizing and restoring is not always doable. When we know it cannot be done in the current environment, just no-op.
  ...
  - New skin (TuiSkin, organized following darkskin, also needs to be added to `Build.ps1 -Project GacUI`)
...
- UI Best Practice
  ...
  - Main reason is to eliminate the usage of `<ImageFrame/>` and control margins, GUI needs margins to make the UI clear, but TUI has big character pixel.
  - Rules for TuiSkin
    - (to be continued)
  - Rules for a good looking layout
    - (to be continued)
